...
初期開発中（`_wip`）。Phase 1（マウントエンジン、認証情報・設定の保存、到達確認）
は実装済み・ユニットテスト済みで、基本的なメニューバー UI が動作します。Phase 2 で
ログイン起動（`SMAppService`）、到達確認付き自動マウント、スリープ復帰／ネットワーク
復帰時の再マウントを組み込みます。詳細は
