APP_NAME    := ShareMounter
NAME        := share-mounter
BUNDLE_ID   := jp.nlink.share-mounter
...
# Hardened Runtime alone suffices. NetFS mounting requires no special entitlement.
...
# is $(APP_NAME).app.
BREW_KIND      := cask
BREW_DESC      := Menu-bar app that auto-mounts SMB shares at login, window-free
BREW_NAME      := $(NAME)
